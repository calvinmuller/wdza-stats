...
Superseded in part by `.scratch/staff-auth/`: the secret URL no longer gates this area. It now lives at `/admin` behind Staff Member sign-in and a Role check (ADR 0005); `ADMIN_PATH_SECRET` remains only for the bootstrap page that creates the first admin and resets an admin's password.
