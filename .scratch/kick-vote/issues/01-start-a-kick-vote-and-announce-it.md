...
- [x] Starting a vote calls the new `broadcast` RCON wrapper with exactly: `Kick vote started against {targetName} - reason: {reason} - vote now: stats.wardogsza.co.za/kick`. If only 1 kick vote is active /kick should list the active vote, it there are multiple (per-server) then we should append /{id} to that. Implemented in `apps/worker/src/kick-vote-engine.ts` (not apps/web - see `no-rcon-access.test.ts`): starting a vote writes the row and `pg_notify`s the Worker, which makes the actual RCON call.
